...
async function checkKeywordRankWithAPI(keyword, blogId, logNo, clientId, clientSecret) {
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (isSameBlog && isSamePost) return { rank: i + 1, found: true, totalChecked: items.length, total: res.data.total || 0 };
    }
    return { rank: null, found: false, totalChecked: items.length, total: res.data.total || 0 };
