...
function isValidTag(raw) {
  return raw && raw.length >= 2 && raw.length <= 20 &&
    !UI_BLACKLIST.has(raw) && /[가-힣a-zA-Z0-9]/.test(raw);
}
...
// HTML 원문에서 #해시태그 정규식으로 직접 추출 (CSS 파싱 실패 시 fallback)
function parseTagsFromRaw(html) {
  const tags = [];

  // JSON 데이터 속 태그 배열 패턴
  const jsonMatch = html.match(/"tagList"\s*:\s*\[([^\]]+)\]/);
  if (jsonMatch) {
  ...
  // "tag":"값" 패턴
  const tagMatches = html.match(/"tag"\s*:\s*"([^"]{2,20})"/g) || [];
  ...
  // #해시태그 직접 패턴 (한글/영문)
  const hashMatches = html.match(/#([가-힣a-zA-Z0-9]{2,15})/g) || [];
